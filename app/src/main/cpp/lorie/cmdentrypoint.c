...
Java_com_micewine_emu_CmdEntryPoint_start(JNIEnv *env, __unused jclass cls, jobjectArray args) {
    ...
    setenv("TMPDIR", "/data/data/io.github.arbuzyonak.cyclone/files/usr/tmp", 1);
    ...
    if (!getenv("XKB_CONFIG_ROOT")) {
        if (access("/data/data/io.github.arbuzyonak.cyclone/files/usr/share/X11/xkb", F_OK) == 0)
            setenv("XKB_CONFIG_ROOT", "/data/data/io.github.arbuzyonak.cyclone/files/usr/share/X11/xkb", 1);
    }
